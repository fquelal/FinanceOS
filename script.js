...
function openEditBill(id){
  ...
  if(zeroHintEl){
    const isZeroBalEdit=(b.btype==='creditcard'||b.btype==='loan')&&!(b.balance>0);
...
function buildAlerts(){
  ...
  data.bills.forEach(b=>{
    ...
    const isZeroBalance=(b.btype==='creditcard'||b.btype==='loan')&&!(b.balance>0);
    ...
    if(d<0){
      ...
      const effAmt=(b.btype==='creditcard'||b.btype==='loan')&&!(b.balance>0)?0:(b.amount||0);
      alerts.push({
...
function renderDashboard(){
  ...
  if(summaryEl){
    const overdueBills=(data.bills||[]).filter(b=>b.status==='Overdue'&&!((b.btype==='creditcard'||b.btype==='loan')&&!(b.balance>0))&&!(b.skipMonths&&b.skipMonths.includes(curMonthStr))).length;
    const parts=[];
...
function renderMoneySummaryCards(){
  ...
  const effBillAmt=b=>(b.btype==='creditcard'||b.btype==='loan')&&!(b.balance>0)?0:(b.amount||0);
...
function renderSnapshot(){
  ...
  const thisBillsOut = data.bills.filter(b => inMonth(b.dueDate, yr, mo) && !((b.btype==='creditcard'||b.btype==='loan')&&!(b.balance>0))).reduce((s,b) => s + (b.scheduledAmount||b.amount||0), 0);
  ...
  const lastBillsOut = data.bills.filter(b => inMonth(b.dueDate, prevYr, prevMo) && !((b.btype==='creditcard'||b.btype==='loan')&&!(b.balance>0))).reduce((s,b) => s + (b.scheduledAmount||b.amount||0), 0);
...
function exportToExcel(){
  ...
  const totalIncome   = data.income.reduce((s,i)=>s+getEffectiveIncome(i,new Date().getFullYear(),new Date().getMonth()),0);
  const totalExpenses = data.expenses.reduce((s,e)=>s+e.amount,0);
  const totalBills    = data.bills.filter(b=>!((b.btype==='creditcard'||b.btype==='loan')&&!(b.balance>0))).reduce((s,b)=>s+(b.scheduledAmount||b.amount||0),0);
  const totalDebt     = data.bills.filter(b=>b.btype==='creditcard'||b.btype==='loan').reduce((s,b)=>s+(b.balance||0),0);

...
function confirmPurchase(){
  ...
  if(method.startsWith('card-')){
    ...
    if(card){
      ...
      // A charge on a card whose due date is stuck in the past (e.g. it sat at
      // $0 and never got paid/reset) should snap its due date back into the
      // current cycle rather than staying frozen and racking up "Nd overdue".
      if(card.recurring&&card.recurring!=='One-time'&&card.dueDate&&daysUntil(card.dueDate)<0){
...
function advanceDueDate(dueDate, recurring){
  ...
  do{
    ...
    else return dueDate; // One-time or unrecognized: leave as-is
    steps++;
